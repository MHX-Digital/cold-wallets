...
STATES={"RECEIVED","VALIDATED","BROADCASTING","BROADCAST","CONFIRMED","FAILED_RETRYABLE","FAILED_TERMINAL"}
TRANSITIONS={"RECEIVED":{"VALIDATED","FAILED_TERMINAL"},"VALIDATED":{"BROADCASTING","FAILED_TERMINAL"},"BROADCASTING":{"BROADCAST","FAILED_RETRYABLE","FAILED_TERMINAL"},"FAILED_RETRYABLE":{"BROADCASTING","FAILED_TERMINAL"},"BROADCAST":{"CONFIRMED"},"CONFIRMED":set(),"FAILED_TERMINAL":set()}
    ...
    def _initialize(self):
        ...
        try:
            with self._connect() as db:
                ...
                db.execute("CREATE TABLE IF NOT EXISTS operations(tx_hash TEXT PRIMARY KEY,network TEXT NOT NULL,state TEXT NOT NULL,retries INTEGER NOT NULL DEFAULT 0,updated_at INTEGER NOT NULL)")
        except sqlite3.DatabaseError as exc: raise BroadcastStateError("invalid broadcast database") from exc
    def receive_bitcoin(self,raw_hex: str,network: str="bitcoin-mainnet"):
        tx_hash=bitcoin_txid(raw_hex); now=int(time.time())
        with self._lock,self._connect() as db:
            db.execute("BEGIN IMMEDIATE")
            db.execute("INSERT OR IGNORE INTO operations(tx_hash,network,state,updated_at) VALUES(?,?,?,?)",(tx_hash,network,"RECEIVED",now))
